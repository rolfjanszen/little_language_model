...
class FeedForward(nn.Module):
    ...
    def __init__(self, input_len):
        super().__init__()
        mid_size = int(input_len*4)
        self.linear1 = nn.Linear(input_len, mid_size, bias=False)
        self.linear2 = nn.Linear(mid_size, input_len, bias=False)
...
class Block(nn.Module):
    def __init__(self, input_len, token_depth, nr_heads, masked):
        super().__init__()
        self.multi_head1 = MultiHeadAttention(input_len, token_depth, nr_heads, masked)
        self.add_norm = AddAndNorm(input_len)
        self.feed_forward = FeedForward(input_len)
        self.add_norm3 = AddAndNorm(input_len)
...
class Decoder(nn.Module):
    ...
    def __init__(self, token_len, token_depth, nr_heads):
        ...
        for i in range(7):
            module_list.append(
                Block(token_len, token_depth, nr_heads, masked=True))
...
class LLM(nn.Module):
    ...
    def forward(self, x):
        ...
        pos_embed = self.pos_embedding(
            torch.arange(x.shape[1]).cuda()).unsqueeze(0)
